...
struct path* get_shortest_path(struct graph_t* G,size_t M, int src,enum color_t id){ 
  ...
  parent[src]=-1;
	...
	for (int count = 0; count < (int)(G->num_vertices - 1); count++) { 
		...
		size_t u = minDistance(dist, sptSet,G->num_vertices); 
      ...
      //printf("heeeere?\n");
        ...
        struct neighbours* nb =get_neighbours(G,v);
        int mn=INT_MA;
        int ind=u;
        for(size_t i=0;i<nb->size;i++){
          if(!sptSet[ind] && nb->nb[i]!=u && dist[nb->nb[i]]<mn){
            mn=dist[nb->nb[i]];
            ind=nb->nb[i];
          }
        }
        nb__free(nb);
        dist[v] = dist[ind] + (int)(gsl_spmatrix_get(G->t,ind,v)-gsl_spmatrix_get(G->o,id,v)*gsl_spmatrix_get(G->o,id,ind));
        parent[v]=ind; 
        }
    }
  }
...
int is_empty(struct graph_t* G, int i){
    if(i<0)
      return 0;
    else if(gsl_spmatrix_get(G->o,0, i) == 0 && gsl_spmatrix_get(G->o,1, i)==0)
      return 1;
    else
      return 0;
}
...
struct move_t play(struct move_t previous_move){
    ...
    player1.N--;
    struct move_t m={0,player1.couleur};
    m.m=0;
    m.c=player1.couleur;
    size_t N = player1.N;
    if(N==0){
      ...
      p_path=get_shortest_path(player1.graph,player1.M,player1.couleur*(player1.M+1),player1.couleur);
      struct path* op_path=NULL;
      op_path=get_shortest_path(player1.graph,player1.M,(player1.couleur+1)%2*(player1.M+1),(player1.couleur+1)%2);
      struct path* un=NULL;
      un=unio(p_path,op_path);
      struct neighbours* nb=NULL;
      nb=get_neighbours(player1.graph,previous_move.m);
      struct path* p=path__empty();
      for(size_t i=0; i<nb->size;i++){
        path__add(p,nb->nb[i]);
      }
      nb__free(nb);
      struct path* uni=NULL;
      uni=unio(un,p);
      path__free(p);
